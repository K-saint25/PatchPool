...
export class IssueWorkflow {
  constructor({
    ...
    leaseOwnerSessionId = randomUUID(),
    ...
    if (!Number.isInteger(leaseOwnerPid) || leaseOwnerPid <= 0 || typeof leaseOwnerSessionId !== 'string' || !leaseOwnerSessionId || !Number.isFinite(externalOperationTimeoutMs) || externalOperationTimeoutMs <= 0) {
      throw new TypeError('IssueWorkflow requires owner process identity and a positive external operation timeout');
    }
    this.leaseOwnerPid = leaseOwnerPid;
    this.leaseOwnerSessionId = leaseOwnerSessionId;
    this.externalOperationTimeoutMs = externalOperationTimeoutMs;
  }
  ...
  assertLease(controller) {
    ...
    if (controller.error) {
      controller.abortController.abort(controller.error);
  ...
  async withLease(controller, operation) {
    ...
    try {
      const result = await operation(controller.abortController.signal);
      this.assertLease(controller);
      return result;
    } catch (error) {
      this.assertLease(controller);
  ...
  startLeaseHeartbeat(lease) {
    const controller = { lease, error: null, timer: null, abortController: new AbortController() };
  ...
  async openPullRequest({ fullName, base, branch, number, title, publish, signal, timeoutMs }) {
    const options = { signal, timeoutMs };
    let found = await this.github.findPullRequest(fullName, branch, options);
    ...
    let created;
    try {
      created = await this.github.createDraftPullRequest({ repository: fullName, branch, base, title: title ?? `Resolve issue #${number}`, body }, options);
    } catch (createError) {
      const reconciled = await this.github.findPullRequest(fullName, branch, options).catch(() => null);
  ...
  async run({ repo, issueNumber: requestedIssueNumber, publish = false, keepWorkspace = false } = {}) {
    ...
    try {
      ...
      if (claim.state === 'pushed') {
        pr = await this.withLease(leaseController, signal => this.openPullRequest({ fullName, base, branch, number, title: refreshed.issue.title, publish, signal, timeoutMs: this.externalOperationTimeoutMs }));
      ...
      await this.withLease(leaseController, signal => this.invoke('git', ['-c', `core.hooksPath=${hooksDirectory}`, 'push', '--set-upstream', remoteName, branch], { cwd: workspace, env: sanitizeEnvironment(this.environment, { push: true }), signal, timeoutMs: this.externalOperationTimeoutMs }, 'WORKFLOW_PUSH_FAILED', 'Unable to push the worker branch'));
      ...
      pr = await this.withLease(leaseController, signal => this.openPullRequest({ fullName, base: beforePush.repository.defaultBranch ?? base, branch, number, title: beforePush.issue.title ?? issue.title, publish, signal, timeoutMs: this.externalOperationTimeoutMs }));
      claim = await this.transition(claim, 'pr_opened', { prUrl: pr.url, openedAt: this.clock() }, leaseController);
